...
Система логирования построена на `loguru` (`utils/logger.py`) и пишет логи в volume‑директорию
`/app/logs` внутри контейнера (или `./logs` при локальном запуске). Настроена ротация по
размеру (`10 MB`) и retention (`7 days`), старые файлы автоматически сжимаются в zip‑архивы.
Декоратор `log_execution` добавляет подробные входные/выходные логи для функций и методов,
при этом потенциально чувствительные kwargs (ключи, токены, пароли) автоматически
редактируются (`<redacted>`), чтобы не допускать утечки секретов в логи.
